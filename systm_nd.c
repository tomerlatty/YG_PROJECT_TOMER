...
void node_set_vector(struct vector *vec, struct vector *root_vec) // set the global node_vector to point the current vec 
{
    node_vector = vec;
    node_vector_root = root_vec;
}

void node_push(struct node *node) // push a node to the end of the  vector 
{
    vector_push(node_vector, &node);
}

struct node *node_peek_or_null() // grab from the back of the vector 
{
    return vector_back_ptr_or_null(node_vector);
}

struct node *node_peek() // peek from the back of the vector 
{
    return *(struct node **)(vector_back(node_vector));
}

struct node *node_pop() // remove from the back of the vector 
{
...
bool node_is_expressionable(struct node *node) // return us if a node is a EXPRESSION_PARENTHESES, EXPRESSION_PARENTHESES or NODE_TYPE_UNARY or NODE_TYPE_IDENTIFIER or NODE_TYPE_NUMBER or NODE_TYPE_STRING;
{
...
struct node *node_peek_expressionable_or_null() // return us the node_is_expressionable or null 
{
...
// end of the making functions 
//----------------------------------------------------

struct node *node_from_sym(struct symbol *sym) // return us the data of the symbol or null 
{
...
struct node *node_from_symbol(struct compile_process *current_process, const char *name) // return us the node of the symbol we look for 
{
...
struct node *struct_node_for_name(struct compile_process *current_process, const char *name) // return us the node of the symbol if its a struct node 
{
...
struct node *union_node_for_name(struct compile_process *current_process, const char *name)// return us the node of the symbol if its a union node 
{
    struct node *node = node_from_symbol(current_process, name);
    if (!node)
    {
...
struct node *node_create(struct node *_node) // create a new node in the system 
{
...
bool node_is_struct_or_union_variable(struct node *node) // return us if the node is struct or union 
{
...
struct node *variable_node(struct node *node) // return us the variable of the node 
{
    ...
    /*
        ...
        so var_name is the variable of the struct 
    */
...
bool variable_node_is_primitive(struct node *node) // return us if the node is a primitive type node 
{
    assert(node->type == NODE_TYPE_VARIABLE);
    return datatype_is_primitive(&node->var.type);
}

struct node *variable_node_or_list(struct node *node) // return us if the node if its in a list , or the variable of the node 
{
...
size_t function_node_argument_stack_addition(struct node *node) // return us how much to add to the eBP to find the first argument. (in assembly stack we add)
{
    assert(node->type == NODE_TYPE_FUNCTION);
    return node->func.args.stack_addition;
}

bool node_is_expression_or_parentheses(struct node *node) // return us if a node is a expression or parentheses
{
    return node->type == NODE_TYPE_EXPRESSION_PARENTHESES || node->type == NODE_TYPE_EXPRESSION;
}

bool node_is_value_type(struct node *node) // return us if a node is a expression or parentheses or identifier ot number or unary or tenary or string 
{
...
bool node_is_expression(struct node *node, const char *op) // return us id the node is a node type expression 
{
    return node->type == NODE_TYPE_EXPRESSION && S_EQ(node->exp.op, op);
}

bool is_array_node(struct node *node) // return us if the node is array node 
{
    return node_is_expression(node, "[]");
}

bool is_node_assignment(struct node *node) // return us if the op of the node is = += -= /= *= 
{
