...
struct parser_scope_entity *parser_new_scope_entity(struct node *node, int stack_offset, int flags) // craete a new parser_scope_entity
{
...
struct parser_scope_entity *parser_scope_last_entity_stop_global_scope() // return us the last entity of the current process 
{
...
struct history *history_begin(int flags) // create a new hisory struct 
{
...
struct history *history_down(struct history *history, int flags) // clown a history
{
...
struct parser_history_switch parser_new_switch_statement(struct history *history) // return us the intilaize parser_history_switch of a history object 
{
...
void parser_scope_new() // create a new scope with the current process 
{
    scope_new(current_process, 0);
}

void parser_scope_push(struct parser_scope_entity *entity, size_t size) // push a entity to the scope system 
{
    scope_push(current_process, entity, size);
}

void parser_scope_finish() // finish a scope process . all the function here is similar to the scope.c functions but work with the current process 
{
    scope_finish(current_process);
}

struct parser_scope_entity *parser_scope_last_entity() // return us the last entity of the  scope 
{
    return scope_last_entity(current_process);
}

static void parser_ignore_nl_or_comment(struct token *token) // skip the tokens that are comment or newline 
{
...
static struct token *token_next() // grab the next token to process from the current process token vector 
{
...
static struct token *token_peek_next() // peek the next token from the vector of tokens 
{
    struct token *next_token = vector_peek_no_increment(current_process->token_vec);
    parser_ignore_nl_or_comment(next_token);
...
static bool token_next_is_operator(const char *op) // return us if the token next is an operator token 
{
    struct token *token = token_peek_next();
    return token_is_operator(token, op);
}

static bool token_next_is_keyword(const char *keyword) // return us if the token next is an keyword token 
{
    struct token *token = token_peek_next();
    return token_is_keyword(token, keyword);
}

static bool token_next_is_symbol(char c) // return us if the token next is an symbol  token 
{
    struct token *token = token_peek_next();
    return token_is_symbol(token, c);
}

static void expect_sym(char c) // we know that the next token should be the c symbol token , so if its not what we expect , system_error
{
    struct token *next_token = token_next();
    if (!next_token || next_token->type != TOKEN_TYPE_SYMBOL || next_token->cval != c)
    {
        system_error(current_process, "Expecting symbol %c however something else was provided \n", c);
    }
}

static void expect_op(const char *op)// we know that the next token should be the op operator token , so if its not what we expect , system_error
{
    struct token *next_token = token_next();
    if (!next_token || next_token->type != TOKEN_TYPE_OPERATOR || !S_EQ(next_token->sval, op))
    {
        system_error(current_process, "Expecting the operator %s but something else was provided\n", next_token->sval);
    }
}

static void expect_keyword(const char *keyword) // we know that the next token should be the 'keyword' keyword token , so if its not what we expect , system_error
{
    struct token *next_token = token_next();
    if (!next_token || next_token->type != TOKEN_TYPE_KEYWORD || !S_EQ(next_token->sval, keyword))
    {
        system_error(current_process, " Expecting the keyword %s but something else was provided\n", keyword);
    }
}

void parse_single_token_to_node() // parse a token to node depended the type of the token 
{
    struct token *token = token_next();
    struct node *node = NULL;
    switch (token->type)
    {
    case TOKEN_TYPE_NUMBER: // number node 
        node = node_create(&(struct node){.type = NODE_TYPE_NUMBER, .llnum = token->llnum});
        break;
    case TOKEN_TYPE_IDENTIFIER: // identifier node 
        node = node_create(&(struct node){.type = NODE_TYPE_IDENTIFIER, .sval = token->sval});
        break;
    case TOKEN_TYPE_STRING: // string  node 
        node = node_create(&(struct node){.type = NODE_TYPE_STRING, .sval = token->sval});
        break;

    default:
        system_error(current_process, "this is not a single token that can be converted to a node");
    }
}

void parse_expressionable_for_op(struct history *history, const char *op) // create a node for a op token 
{
    parse_expressionable(history);
}

static int parser_get_precedence_for_operator(const char *op, struct expressionable_op_precedence_group **group_out) // return us the precedence index (in the array) of an operator 
{
    *group_out = NULL;
...
static bool parser_left_op_has_priority(const char *op_left, const char *op_right) // return us if the left op hs more higher priotiry then the right op 
{
...
void parser_node_shift_children_left(struct node *node) // shift left the children of a node 
{
...
void parser_node_move_right_left_to_left(struct node *node) // move right ledt condition to left , a node 
{
...
void parser_reorder_expression(struct node **node_out) // retorder the sub-tree of a node expression 
{
...
void parse_exp_normal(struct history *history) // make a tree for a expression node 
{
...
void parser_deal_with_additional_expression()  // deal with the next expression 
{
...
void parse_for_parentheses(struct history *history) // parse the () parentheses 
{
    expect_op("(");
...
void parse_for_comma(struct history *history) // parse the comma 
{
    // skip the comma
    token_next();
...
void parse_for_array(struct history *history) // parse the array
{
...
void parse_for_cast() // parse the casting 
{
...
int parse_exp(struct history *history) // parse expression send them to functions according to there type 
{
...
void parse_identifier(struct history *history) // parse a identifier token 
{
    assert(token_peek_next()->type == TOKEN_TYPE_IDENTIFIER);
    parse_single_token_to_node();
}
static bool is_keyword_variable_modifier(const char *val) // return if the variable keyword is modifier - unsigned signed static const extern __ignore_typecheck__
{
...
void parse_datatype_modifiers(struct datatype *dtype) // parse the modifiers tokens, send them to functions according to there type 
{
    struct token *token = token_peek_next();
...
void parser_get_datatype_tokens(struct token **datatype_token, struct token **datatype_secondary_token) // function to get the primitive keyword token (reorder)
{
...
int parser_datatype_expected_for_type_string(const char *str) // function to get the enum value of a string, we expect an union or struct
{
...
int parser_get_random_type_index() // for building a random type name 
{
    static int x = 0;
    x++;
    return x;
...
struct token *parser_build_random_type_name() //return us a random token -  type idenfidier
{
    char tmp_name[25];
...
int parser_get_pointer_depth()// to get the depth of a pointer : int **a - so to depth is 2
{
    int depth = 0;
...
bool parser_datatype_is_secondary_allowed(int expected_type) // return us if the expected type is DATA_TYPE_EXPECT_PRIMITIVE
{
    return expected_type == DATA_TYPE_EXPECT_PRIMITIVE;
}

bool parser_datatype_is_secondary_allowed_for_type(const char *type) // return us if the str is long short double float 
{
...
void parser_datatype_adjust_size_for_secondary(struct datatype *datatype, struct token *datatype_secondary_token) // do the changes for the first datatype if a second datatype is exist 
{
...
void parser_datatype_init_type_and_size_for_primitive(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out) // initilaize the datatypes 
{
    if (!parser_datatype_is_secondary_allowed_for_type(datatype_token->sval) && datatype_secondary_token)
    {
        system_error(current_process, "Your not allowed a secondary datatype here for the given datatype%s\n", datatype_token->sval);
    }
    ...
    {
        system_error(current_process, "BUG: Invalid primitive datatype \n");
    }

    parser_datatype_adjust_size_for_secondary(datatype_out, datatype_secondary_token);
}

size_t size_of_struct(const char *struct_name) // return us the size of the struct 
{
...
size_t size_of_union(const char *union_name)// return us the size of the union 
{
...
void parser_datatype_init_type_and_size(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out, int pointer_depth, int expected_type)// do the initilaizes for datatypes
{
    if (!parser_datatype_is_secondary_allowed(expected_type) && datatype_secondary_token)
    {
        system_error(current_process, "You provided an invalid secondary datatype\n");
    }
    ...
    default:
        system_error(current_process, "BUG: Unsuppoerted datatype expectation \n");
    }
}

void parser_datatype_init(struct token *datatype_token, struct token *datatype_secondary_token, struct datatype *datatype_out, int pointer_depth, int expected_type)// do the initilaizes for datatypes the main function of this subject 
{
    ...
    {
        system_warning(current_process, " our compiler does not support 64 bit longs, therefore your long long is defaulting to 32 bits \n");
        datatype_out->size = DATA_SIZE_DWORD;
    }
}

void parse_datatype_type(struct datatype *dtype) // function to parse datatype 
{
...
void parse_datatype(struct datatype *dtype) // the main function of parse datatype 
{
...
bool parser_is_int_valid_after_datatype(struct datatype *dtype) // return us if the datatype is LONG FLOAT or DOUBLE 
{
    return dtype->type == DATA_TYPE_LONG || dtype->type == DATA_TYPE_FLOAT || dtype->type == DATA_TYPE_DOUBLE;
}

/*long int abc*/
void parser_ignore_int(struct datatype *dtype) // ignore the int token in cases like long int abc 
{
    ...
    {
        system_error(current_process, "you provided a secondary \"int\" type however its not supported with this current abbveration \n ");
    }
...
void parse_expressionable_root(struct history *history) // parse a root expressionable 
{
    parse_expressionable(history);
...
struct datatype_struct_node_fix_private // thye struct of datatype_struct_node_fix_private 
{
    // node to fix.
    struct node *node;
};

bool datatype_struct_node_fix(struct fixup *fixup) // fix the struct node that we need to fix (fill in the node data)
{
...
void datatype_struct_node_end(struct fixup *fixup) // free the memory of the struct that we fixed 
{
    free(fixup_private(fixup));
}

void make_variable_node(struct datatype *dtype, struct token *name_token, struct node *value_node) // function to make a variable node 
{
    ...
    if (var_node->var.type.type == DATA_TYPE_STRUCT && !var_node->var.type.struct_node) // if we dont know him we need to add thim to the fixup system 
    {
...
void parser_scope_offset_for_stack(struct node *node, struct history *history) // set the offset for the stack 
{
...
void parser_scope_offset_for_global(struct node *node, struct history *history) // we will not deal with that in the project 
{
}

void parser_scope_offset_for_structure(struct node *node, struct history *history) // calculate and set the affset for structure node 
{
    int offset = 0;
...
void parse_scope_offset(struct node *node, struct history *history) // the main function of setting up the offsets 
{
...
void make_variable_node_and_register(struct history *history, struct datatype *dtype, struct token *name_token, struct node *value_node)// make a variable node and register him to the systems 
{
    make_variable_node(dtype, name_token, value_node);
    struct node *var_node = node_pop();

    // calculate the scope offset
    parse_scope_offset(var_node, history);
...
void make_variable_list_node(struct vector *var_list_vec) // make a list type node - node->node->node
{
...
void parse_variable(struct datatype *dtype, struct token *name_token, struct history *history) // function to parse a variable token 
{
...
void parse_function_body(struct history *history) // parse the body of a function - int a(){***********}
{
    parse_body(NULL, history_down(history, history->flags | HISTORY_FLAG_INSIDE_FUNCTION_BODY));
}

void parse_function(struct datatype *ret_type, struct token *name_token, struct history *history) // the main function of parsing a function  *int a(){}*
{
...
void parse_symbol() // parse a symbol token 
{
    if (token_next_is_symbol('{'))
    {
    ...
    else if (token_next_is_symbol(':')) // ; case
    {
        parse_label(history_begin(0));
        return;
    }

    system_error(current_process, "Invalid symbol was provided");
}

void parse_statement(struct history *history) // parse a statement token 
{
    if (token_peek_next()->type == TOKEN_TYPE_KEYWORD)
    {
...
void parser_append_size_for_node_struct_union(struct history *history, size_t *_variable_size, struct node *node) // append the size for a sruct node or union 
{
...
void parser_append_size_for_variable_list(struct history *history, size_t *variable_size, struct vector *vec) // append size for list of nodes 
{
...
void parser_append_size_for_node(struct history *history, size_t *_variable_size, struct node *node) // append the size for a node 
{
    if (!node)
    {
        return;
...
void parser_finalize_body(struct history *history, struct node *body_node, struct vector *body_vec, size_t *_variable_size, struct node *largest_align_eligible_var_node, struct node *largest_possible_var_node)// do all the computing and the padding, align value for a body 
{
...
void parse_body_single_statement(size_t *variable_size, struct vector *body_vec, struct history *history) // do the parsing of a prototype 
{
    ...
    // change the variable _size variable by the size of stmt_node
...
void parse_body_multiple_statements(size_t *variable_size, struct vector *body_vec, struct history *history)// we have some statements between curly braces { int a; int b; int c;} - so we handle this 
{
...
// set to the sum of all variables sizes encountered in the parsed body
void parse_body(size_t *variable_size, struct history *history)
{
    parser_scope_new();
...
void parse_struct_no_new_scope(struct datatype *dtype, bool is_forward_declaration) // parse a struct but not need to open a new scope , like prototype cases 
{
    ...
    // all structures ends with semicolons
    expect_sym(';');
...
void parse_union_no_scope(struct datatype *dtype, bool is_forward_declaration)// parse a union but not need to open a new scope , like prototype cases 
{
    struct node *body_node = NULL;
    size_t body_variable_size = 0;
    if (!is_forward_declaration)
...
void parse_union(struct datatype *dtype) // parse a union 
{
...
void parse_struct(struct datatype *dtype) // parse a structure 
{
    bool is_forward_declaration = !token_is_symbol(token_peek_next(), '{');
    if (!is_forward_declaration)
    {
...
void parse_struct_or_union(struct datatype *dtype) // get a datatype and decide which function to swnt the datatype - struct or union functions 
{
    switch (dtype->type)
    {
    ...
    default:
        system_error(current_process, "COMPILER BUG: The provided datatype is not a structure or union\n");
    }
}

void token_read_dots(size_t amount) // expect for how many dots we get in the parameter 
{
...
void parse_variable_full(struct history *history) // the main function of parsing a variable 
{
    struct datatype dtype;
    parse_datatype(&dtype);
...
struct vector *parse_function_arguments(struct history *history) // parsing a arguments that function get and return us the vector of the nodes  
{
    parser_scope_new();
    struct vector *arguments_vec = vector_create(sizeof(struct node *));
    while (!token_next_is_symbol(')'))
    {
        if (token_next_is_operator(".")) // like printf parameters 
        {
            token_read_dots(3);
            parser_scope_finish();
...
void parse_forward_declaration(struct datatype *dtype) // since this is a forward declaration , parse the structure
{
    parse_struct(dtype);
}

void parse_variable_function_or_struct_union(struct history *history) // the main function of parsing a struct or a function , prototype or the full element 
{
    struct datatype dtype;
    parse_datatype(&dtype);
    ...
    // ignore integer abbrevations if necceasry i.e "long int" becomes just "long"
    ...
    {
        system_error(current_process, "Expecting a valid name for the given variable declaration\n");
    }
...
struct node *parse_else(struct history *history) // parse else node 
{
    size_t var_size = 0;
...
struct node *parse_else_or_else_if(struct history *history)// parse else node or else if node 
{
    struct node *node = NULL;
...
void parse_if_stmt(struct history *history) // parse if node 
{
    expect_keyword("if");
    expect_op("(");
    // cond
...
void parse_keyword_parentheses_expression(const char *keyword) //parse the data inside a parentheses keyword - keyword(**)
{
...
void parse_case(struct history *history) // parse the case in a switch case 
{
    expect_keyword("case");
    parse_expressionable_root(history);
    ...
    {
        system_error(current_process, "we only support numbers in our subset of c \n");
    }
...
void parse_switch(struct history *history)// parse the switch  in a switch case 
{
...
void parse_do_while(struct history *history) // parse the do while option (node) 
{
    expect_keyword("do");
    size_t var_size = 0;
    parse_body(&var_size, history);
...
void parse_while(struct history *history) // parse the while node 
{
    parse_keyword_parentheses_expression("while");
    struct node *exp_node = node_pop();
    size_t variable_size = 0;
...
bool parse_for_loop_part(struct history *history) // parse the for loop option (node) 
{
    if (token_next_is_symbol(';'))
    {
        // we have nothing here i.t "for (;"
...
bool parse_for_loop_part_loop(struct history *history) // parse the loop in the for loop (intilaize;cond;*loop*)
{
    if (token_next_is_symbol(')'))
    {
        return false;
...
void parse_for_stmt(struct history *history) // parse the for statement for(intilaize;cond;loop){}
{
...
void parse_return(struct history *history) // parse the return node 
{
...
void parse_continue(struct history *history) // parse the continue  node 
{
...
void parse_break(struct history *history) // parse the break  node 
{
...
void parse_goto(struct history *history) // parse the goto node 
{
    expect_keyword("goto");
...
void parse_label(struct history *history) // parse the label  node 
{
    expect_sym(':');
    struct node *label_name_node = node_pop();
    if (label_name_node->type != NODE_TYPE_IDENTIFIER)
    {
        system_error(current_process, "Expecting an identifier for labels something else was provided");
    }

    make_label_node(label_name_node);
}

void parse_for_tenary(struct history *history)  // parse the tenary node - if?option1,option2 
{
...
void parse_keyword(struct history *history) // the main function of sending every keyword token to the function that parsing him 
{
    struct token *token = token_peek_next();
    ...
    system_error(current_process, "Invalid keyword\n");
}

int parse_expressionable_single(struct history *history) // send the tokens to there functions that create thier nodes 
{
    struct token *token = token_peek_next();
    if (!token)
    {
    ...
    history->flags |= NODE_FLAG_INSIDE_EXPRESSION; // if we inside expression 5+3
    int res = -1;
    switch (token->type)
    {
...
void parse_expressionable(struct history *history) // go through all of the tokens , create nodes out of them 
{
    while (parse_expressionable_single(history) == 0) // it will run until the function parse_expressionable_single return 0
    {
    }
}

void parse_keyword_for_global() // the function that parse the keyword by parse_keyword and push the node 
{
...
int parse_next() // parse the next token depended the token type 
{
    struct token *token = token_peek_next(); // peek the next token
    if (!token) // if there no next token 
    {
        return -1;
    }

    int res = 0;
    switch (token->type) // switch case , to decide what is the type of the node 
    {
    case TOKEN_TYPE_NUMBER:
    case TOKEN_TYPE_IDENTIFIER:
...
int parse(struct compile_process *process) // the main function of the parser part 
{
    scope_create_root(process); // create a new root scope 
